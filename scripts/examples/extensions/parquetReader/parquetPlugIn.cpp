...
static std::shared_ptr<arrow::Array> cast_array_if_needed(const std::shared_ptr<arrow::Array> &arr,
...
static std::string materialize_dict_value(const std::shared_ptr<arrow::Array> &dict, int64_t code) {
    ...
    case T::BINARY: {
        ...
        if (!(code >= 0 && code < d->length()) || !d->IsValid(code))
            return {};
    ...
    default: {
        ...
        if (sres.ok() && sres.ValueOrDie())
            return sres.ValueOrDie()->ToString();
        return {};
    }
    }
}
...
void parquet_read(Structure *&res, const FileMetaData &fmd, const char *filename, IOOptions &opts, DaphneContext *ctx) {
    ...
    if (rgOffset[numRG] != rows)
        throw std::runtime_error("row count mismatch: parquet=" + std::to_string(rgOffset[numRG]) +
                                 " expected(meta)=" + std::to_string(rows));
                ...
                try {
                    ...
                    for (int rg = rg0; rg < rg1; ++rg) {
                        ...
                        PARQUET_THROW_NOT_OK(rdr->ReadRowGroup(rg, all_cols, &tbl));
                        {
                            ...
                            if (!cc.ok())
                                throw std::runtime_error(cc.status().ToString());
                            tbl = cc.ValueOrDie();
                            auto fl = tbl->Flatten();
                            if (!fl.ok())
                                throw std::runtime_error(fl.status().ToString());
                            tbl = fl.ValueOrDie();
                        }
                            ...
                            for (int k = 0; k < chunked->num_chunks(); ++k) {
                                ...
                                if (arr->null_count() == 0) {
                                    ...
                                    for (int64_t i = 0; i < len; ++i)
                                        dst_col0[i * cols] = src[i];
                                } else {
                                    for (int64_t i = 0; i < len; ++i)
                                        dst_col0[i * cols] = arr->IsValid(i) ? arr->Value(i) : 0;
                                }
                                logical_row += len;
                            }
                            if (logical_row != nRG)
                                throw std::runtime_error("RG row mismatch (int8)");
                        }
                    }
                } catch (...) {
            ...
            workers.emplace_back([&, rg0, rg1]() {
                try {
                    ...
                    for (int rg = rg0; rg < rg1; ++rg) {
                        ...
                        PARQUET_THROW_NOT_OK(rdr->ReadRowGroup(rg, all_cols, &tbl));
                        if (!tbl)
                            throw std::runtime_error("ReadRowGroup returned null Table");

                        // Combine chunks + flatten (structs -> leaves as fields)
                        {
                            auto cc = tbl->CombineChunks(pool);
                            if (!cc.ok())
                                throw std::runtime_error("CombineChunks failed: " + cc.status().ToString());
                            tbl = cc.ValueOrDie();

                            auto fl = tbl->Flatten();
                            if (!fl.ok())
                                throw std::runtime_error("Flatten failed: " + fl.status().ToString());
                            tbl = fl.ValueOrDie();
                        }
                ...
                try {
                    ...
                    for (int rg = rg0; rg < rg1; ++rg) {
                        ...
                        for (int c = 0; c < cols; ++c) {
                            ...
                            if (logical_row != nRG)
                                throw std::runtime_error("RG row mismatch (int64)");
                        }
                    }
                } catch (...) {
                    std::lock_guard<std::mutex> g(mx);
                        ...
                        for (int c = 0; c < cols; ++c) {
                            ...
                            for (int k = 0; k < chunked->num_chunks(); ++k) {
                                ...
                                auto arr = std::static_pointer_cast<arrow::UInt8Array>(
                                    cast_array_if_needed(arr0, arrow::uint8(), pool));
                                const int64_t len = arr->length();
                            ...
                            if (logical_row != nRG)
                                throw std::runtime_error("RG row mismatch (uint8)");
                        }
                    }
                } catch (...) {
                    std::lock_guard<std::mutex> g(mx);
                        ...
                        for (int c = 0; c < cols; ++c) {
                            ...
                            for (int k = 0; k < chunked->num_chunks(); ++k) {
                                ...
                                auto arr = std::static_pointer_cast<arrow::UInt32Array>(
                                    cast_array_if_needed(arr0, arrow::uint32(), pool));
                                const int64_t len = arr->length();
                            ...
                            if (logical_row != nRG)
                                throw std::runtime_error("RG row mismatch (uint32)");
                        }
                    }
                } catch (...) {
                    std::lock_guard<std::mutex> g(mx);
                        ...
                        for (int c = 0; c < cols; ++c) {
                            ...
                            for (int k = 0; k < chunked->num_chunks(); ++k) {
                                ...
                                auto arr = std::static_pointer_cast<arrow::UInt64Array>(
                                    cast_array_if_needed(arr0, arrow::uint64(), pool));
                                const int64_t len = arr->length();
                            ...
                            if (logical_row != nRG)
                                throw std::runtime_error("RG row mismatch (uint64)");
                        }
                    }
                } catch (...) {
                    std::lock_guard<std::mutex> g(mx);
                ...
                try {
                    ...
                    for (int rg = rg0; rg < rg1; ++rg) {
                        ...
                        for (int c = 0; c < cols; ++c) {
                            ...
                            if (it == name2idx.end()) {
                                for (int64_t i = 0; i < nRG; ++i)
                                    data[(base + i) * cols + c] = std::numeric_limits<float>::quiet_NaN();
                                continue;
                            }
                            auto chunked = tbl->column(it->second);
                            if (!chunked) {
                            ...
                            for (int k = 0; k < chunked->num_chunks(); ++k) {
                                ...
                                if (arr->null_count() == 0) {
                                    ...
                                    for (int64_t i = 0; i < len; ++i)
                                        dst_col0[i * cols] =
                                            arr->IsValid(i) ? arr->Value(i) : std::numeric_limits<float>::quiet_NaN();
                                }
                                logical_row += len;
                            }
                            if (logical_row != nRG)
                                throw std::runtime_error("RG row mismatch (float)");
                        }
                    }
                } catch (...) {
                    std::lock_guard<std::mutex> g(mx);
...
void parquet_read_frame(Frame *&res, const FileMetaData &fmd, const char *filename, IOOptions &opts,
                        DaphneContext *ctx) {
    using arrow::io::FileMode;
    using arrow::io::MemoryMappedFile;
    using parquet::ParquetFileReader;
    ...
    for (int c = 0; c < cols; ++c) {
        ...
        case ValueTypeCode::SI8:
            columns[c] = DataObjectFactory::create<DenseMatrix<int8_t>>(rows, 1, true);
            break;
        case ValueTypeCode::SI32:
            columns[c] = DataObjectFactory::create<DenseMatrix<int32_t>>(rows, 1, true);
            break;
        case ValueTypeCode::SI64:
            columns[c] = DataObjectFactory::create<DenseMatrix<int64_t>>(rows, 1, true);
            break;
        case ValueTypeCode::UI8:
            columns[c] = DataObjectFactory::create<DenseMatrix<uint8_t>>(rows, 1, true);
            break;
        case ValueTypeCode::UI32:
            columns[c] = DataObjectFactory::create<DenseMatrix<uint32_t>>(rows, 1, true);
            break;
        case ValueTypeCode::UI64:
            columns[c] = DataObjectFactory::create<DenseMatrix<uint64_t>>(rows, 1, true);
            break;
        case ValueTypeCode::F32:
            columns[c] = DataObjectFactory::create<DenseMatrix<float>>(rows, 1, true);
            break;
        ...
        workers.emplace_back([&, startRG, endRG]() {
            try {
                ...
                for (int rg = startRG; rg < endRG; ++rg) {
                    std::shared_ptr<arrow::Table> tbl;
                    PARQUET_THROW_NOT_OK(reader->ReadRowGroup(rg, all_cols, &tbl));
                    ...
                    for (int c = 0; c < cols; ++c) {
                        ...
                        case ValueTypeCode::SI8: {
                            ...
                            if (arr->null_count() == 0) {
                                ...
                                for (int64_t i = 0; i < nRG; ++i)
                                    dst[base + i] = arr->IsValid(i) ? arr->Value(i) : int8_t(0);
                            }
                            break;
                        }
                        case ValueTypeCode::SI32: {
                            ...
                            if (arr->null_count() == 0) {
                                ...
                                for (int64_t i = 0; i < nRG; ++i)
                                    dst[base + i] = arr->IsValid(i) ? arr->Value(i) : int32_t(0);
                            }
                            break;
                        }
                        case ValueTypeCode::SI64: {
                            ...
                            if (arr->null_count() == 0) {
                                ...
                                for (int64_t i = 0; i < nRG; ++i)
                                    dst[base + i] = arr->IsValid(i) ? arr->Value(i) : int64_t(0);
                            }
                            break;
                        }
                        case ValueTypeCode::UI8: {
                            ...
                            if (arr->null_count() == 0) {
                                ...
                                for (int64_t i = 0; i < nRG; ++i)
                                    dst[base + i] = arr->IsValid(i) ? arr->Value(i) : uint8_t(0);
                            }
                            break;
                        }
                        case ValueTypeCode::UI32: {
                            ...
                            if (arr->null_count() == 0) {
                                ...
                                for (int64_t i = 0; i < nRG; ++i)
                                    dst[base + i] = arr->IsValid(i) ? arr->Value(i) : uint32_t(0);
                            }
                            break;
                        }
                        case ValueTypeCode::UI64: {
                            ...
                            if (arr->null_count() == 0) {
                                ...
                                for (int64_t i = 0; i < nRG; ++i)
                                    dst[base + i] = arr->IsValid(i) ? arr->Value(i) : uint64_t(0);
                            }
                            break;
                        }
                        case ValueTypeCode::F32: {
                        ...
                        case ValueTypeCode::F64: {
                            ...
                            if (arr->null_count() == 0) {
                                ...
                                for (int64_t i = 0; i < nRG; ++i)
                                    dst[base + i] =
                                        arr->IsValid(i) ? arr->Value(i) : std::numeric_limits<double>::quiet_NaN();
                            }
                            break;
                        }
                        default: {
...
static VTTag tag_from_schema(const FileMetaData &fmd) {
    ...
    case ValueTypeCode::STR:
        return VTTag::STR;
    default:
        throw std::runtime_error("parquet_write: unsupported VT in schema[0]");
    }
}

extern "C" void parquet_write(const void *data, const FileMetaData &fmd, const char *filename, const IOOptions &opts,
                              DaphneContext * /*ctx*/
) {
    ...
    case VTTag::F64: {
        ...
        add_numeric_column(
            mat, float64(), []() { return std::make_unique<DoubleBuilder>(default_memory_pool()); },
    ...
    case VTTag::F32: {
        ...
        add_numeric_column(
            mat, float32(), []() { return std::make_unique<FloatBuilder>(default_memory_pool()); },
    ...
    case VTTag::SI64: {
        ...
        add_numeric_column(
            mat, int64(), []() { return std::make_unique<Int64Builder>(default_memory_pool()); },
    ...
    case VTTag::SI32: {
        ...
        add_numeric_column(
            mat, int32(), []() { return std::make_unique<Int32Builder>(default_memory_pool()); },
    ...
    case VTTag::SI8: {
        ...
        add_numeric_column(
            mat, int8(), []() { return std::make_unique<Int8Builder>(default_memory_pool()); },
    ...
    case VTTag::UI64: {
        ...
        add_numeric_column(
            mat, uint64(), []() { return std::make_unique<UInt64Builder>(default_memory_pool()); },
    ...
    case VTTag::UI32: {
        ...
        add_numeric_column(
            mat, uint32(), []() { return std::make_unique<UInt32Builder>(default_memory_pool()); },
    ...
    case VTTag::UI8: {
        ...
        add_numeric_column(
            mat, uint8(), []() { return std::make_unique<UInt8Builder>(default_memory_pool()); },
    ...
    PARQUET_THROW_NOT_OK(
        parquet::arrow::WriteTable(*tbl, default_memory_pool(), out, static_cast<int64_t>(chunk_rows), props));
}

void parquet_write_frame(const void *data, const FileMetaData &fmd, const char *filename, const IOOptions &opts,
                         DaphneContext *ctx) {
    using arrow::Array;
