...
static bool is_numeric(const std::string &s_in) {
    ...
    if (sl == "nan" || sl == "+nan" || sl == "-nan" || sl == "inf" || sl == "+inf" || sl == "-inf")
        return false;
    char *endp = nullptr;
    errno = 0;
    std::strtod(s.c_str(), &endp);
    if (endp == s.c_str() || errno == ERANGE)
        return false;
    while (endp && *endp) {
        ...
        if (st.ok() && arrow_reader) {
            ...
            if (arrow_reader->GetSchema(&schema).ok() && schema) {
                ...
                for (const auto &f : schema->fields()) {
                    ...
                    auto id = f->type()->id();
                    bool is_num = id == T::INT8 || id == T::INT16 || id == T::INT32 || id == T::INT64 ||
                                  id == T::UINT8 || id == T::UINT16 || id == T::UINT32 || id == T::UINT64 ||
                                  id == T::FLOAT || id == T::DOUBLE || id == T::HALF_FLOAT || id == T::DECIMAL128 ||
                                  id == T::DECIMAL256;
                    if (!is_num) {
                        all_numeric = false;
                        break;
...
static bool csv_meta(const std::string &path, long long &rows, long long &cols, std::string &vtype) {
    ...
    while (sampled < MAX_SAMPLE && std::getline(in, line)) {
        ...
        fields.reserve(static_cast<size_t>(cols));
        {
            ...
            std::string tok;
            while (std::getline(ss, tok, ','))
                fields.emplace_back(std::move(tok));
        }
        if (static_cast<long long>(fields.size()) != cols) {
            all_numeric = false;
            break;
        }
...
static bool mtx_meta(const std::string &path, long long &rows, long long &cols, std::string &vtype) {
    ...
    if (hl.find("matrixmarket") == std::string::npos)
        return false;

    if (hl.find(" real ") != std::string::npos)
        vtype = "f64";
    else if (hl.find(" integer ") != std::string::npos)
        vtype = "si32";
    else
        vtype = "str";
