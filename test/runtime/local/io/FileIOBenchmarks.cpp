...
TEMPLATE_PRODUCT_TEST_CASE("FileIO Benchmark double parquet into a densematrix", TAG_IO "[.bench]", (DenseMatrix),
                           (double)) {
...
TEMPLATE_PRODUCT_TEST_CASE("FileIO Benchmark double parquet into a Frame", TAG_IO "[.bench]", (DenseMatrix), (double)) {
    if (!daphne_bench_enabled()) {
        SUCCEED("Benchmarks disabled. Set DAPHNE_BENCH=1 to run.");
        return;
    }
