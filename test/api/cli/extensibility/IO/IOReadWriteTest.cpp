...
bool compareCsvFiles(const std::string &aPath, const std::string &bPath, char delim = ',', bool hasHeader = false,
                     double tol = 0.0, bool nanSafe = false) {
    ...
    while (true) {
        ...
        for (size_t c = 0; c < ca.size(); ++c) {
            ...
            if (tol > 0.0) {
                ...
                if (na && nb) {
                    if (nanSafe && std::isnan(da) && std::isnan(db))
                        continue;
    ...
    TEST_CASE(NAME_LIT "_success", TAG_IO) {                                                                           \
        ...
        for (unsigned i = 1; i <= (COUNT); i++) {                                                                      \
            DYNAMIC_SECTION(name << "_success_" << i) {                                                                \
                std::string scriptPathWrt = dirPath + name + "_success_" + std::to_string(i) + ".daphne";              \
                std::string scriptPathCmp = dirCheckPath + "do_check_" + dt + ".daphne";                               \
                std::string outPath = dirPath + "out/" + name + "_success_" + std::to_string(i) + ".csv";              \
                std::string refPath = dirPath + "ref/" + name + "_success_" + std::to_string(i) + "_ref.csv";          \
                                                                                                                       \
                std::error_code ec;                                                                                    \
                std::filesystem::remove(outPath, ec);                                                                  \
                                                                                                                       \
                checkDaphneStatusCode(StatusCode::SUCCESS, scriptPathWrt.c_str(), "--args",                            \
                                      (std::string("outPath=\"") + outPath + "\"").c_str(), kExtFlag, kExtPath);       \
                                                                                                                       \
    ...
    TEST_CASE(base ", IO failure", TAG_IO) {                                                                           \
        for (unsigned i = 1; i <= count; ++i) {                                                                        \
            DYNAMIC_SECTION(base "_failure_" << i << ".daphne") {                                                      \
                checkDaphneStatusCodeSimple(StatusCode::EXECUTION_ERROR, dirPath, base "_failure", i, kExtFlag,        \
                                            kExtPath);                                                                 \
            }                                                                                                          \
        }                                                                                                              \
    }
