...
const std::unordered_map<std::string, BuiltinParamInfo> &builtinParamInfos() {
    static const std::unordered_map<std::string, BuiltinParamInfo> infos = {
        ...
        {"writeFrame", {{"arg", "filename", "options"}, 3}},
        {"writeMatrix", {{"arg", "filename", "options"}, 3}},
        {"write", {{"arg", "filename", "options"}, 3}},
        {"readMatrix", {{"filename", "options"}, 2}},
        {"readFrame", {{"filename", "options"}, 2}},
...
antlrcpp::Any DaphneDSLBuiltins::build(mlir::Location loc, const std::string &func,
    ...
    if (func == "readMatrix") {
        ...
        if (numArgs == 1) {
            ...
            auto oneByOneMat = builder.create<mlir::daphne::FillOp>(loc, mlir::daphne::MatrixType::get(ctx, strTy),
                                                                    emptyStr, oneIdx, oneIdx);
            ...
            auto cf = builder.create<mlir::daphne::CreateFrameOp>(loc, oneByOneFT, mlir::ValueRange{oneByOneMat},
                                                                  mlir::ValueRange{labelStr});
